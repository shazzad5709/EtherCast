      ...
      <form className="flex flex-col w-full items-center" onSubmit={handleVote}>
        <div className='lg:grid lg:grid-cols-2 xl:grid-cols-3 w-full gap-10 lg:px-10 xl:px-20'>
          {candidates.map((option) => (
            <div key={option.id}
              className={` ${selected === `${option.email}` ? "bg-green hover:bg-green-dark text-white" : "hover:bg-green-light" } w-full border-2 border-gray-300 flex space-x-10 items-center shadow-md space-y-2 rounded-lg mb-4 p-4 cursor-pointer  hover:border-green-light 
                `}
            ...
            >
              ...
              <div className='space-y-2'>
                ...
                <p className={`text-gray-800 text-lg ${selected === `${option.email}` ? "  text-white" : ""}`} >{option.symbol}</p>
                <p className={`text-gray-600 ${selected === `${option.email}` ? "  text-white" : ""}`}>{option.agenda}</p>
              </div>
            </div>
          ))}
        </div>
