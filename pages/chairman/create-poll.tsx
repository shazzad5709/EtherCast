...
import PollForm from "../../components/Utilities/Form/PollForm";
...
export default function CreateElection({}: Props) {
  ...
  const handleCreateElection = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    const gmtRegDeadlineDate = Math.floor(
      new Date(regDeadlineDate.toISOString()).getTime() / 1000
    );
    // const gmtVoteStartDate = new Date(voteStartDate.toISOString()).getTime()
    const gmtVoteStartDate = Math.floor(
      new Date(voteStartDate.toISOString()).getTime() / 1000
    );
    // const gmtVoteEndDate = new Date(voteEndDate.toISOString()).getTime()
    const gmtVoteEndDate = Math.floor(
      new Date(voteEndDate.toISOString()).getTime() / 1000
    );
                ...
                >
                  ...
                  <div>
                    ...
                    <div className="flex space-x-2">
                      <input
                        ...
                        onChange={(e) =>
                          setRegDeadlineDate(new Date(e.target.value))
                        }
                      />
                    </div>
                  </div>
                  <div>
                    ...
                    <input
                      ...
                      onChange={(e) =>
                        setVoteStartDate(new Date(e.target.value))
                      }
                    />
                  </div>
                  <div>
